...
- No CDN dependencies. React/ReactDOM/Babel/marked/hljs are vendored. App must work offline. Babel, marked (`lib/marked.umd.js`) and hljs (`@highlightjs/cdn-assets`) are copied verbatim from npm. React 19 ships no UMD build, so `src/react*.development.js` are generated: `bun scripts/vendor-react.mjs <version>` — never hand-edit them. marked ≥13 passes renderers token objects with *raw* `title`/autolink text, so the link/code renderers in `index.html` escape it themselves.
