...
// ── EvalCell — one kernel cell inside a tool card ────────────────────
// Applies hljs syntax highlighting once the cell is complete. While the
// cell is running the code is a partial string; highlighting it mid-stream
// produces malformed spans, so we stay with plain monospace until done.
      ...
      <div className={`tool-card ${running ? "running" : "ok"}`}>
        ...
        {msg.tool === "eval" && msg.cells && (
          <div className="tool-eval">
            {msg.cells.map((cell, i) => <EvalCell key={i} cell={cell} />)}
          </div>
        )}
      </div>
    </div>
